...
int main() {
	char line[128];
	long long items[8][100]; 
    int operator[8], opValue[8], test[8], trueMonkey[8], falseMonkey[8], itemCount[8] = {0}, monkeyCount = 0, inspections[8] = {0}, monkeys = 8;
		...
		while( fgets(line, sizeof(line), input) != NULL) {
			...
			if(line[2] == 'S') {
				...
				while((token = strtok_r(rest, ",", &rest)) != NULL) {
					items[monkeyCount][itemCount[monkeyCount]] = (long)strtol(token,&temp,10);
	...
	for( int round = 1; round <= 10000; round++) {
		for( int i = 0; i< monkeys; i++) {
			...
			for( int j = 0; j < count; j++) {
				...
				items[tempIndex][itemCount[tempIndex]] = items[i][j] % mod;
                itemCount[tempIndex]++;
				itemCount[i]--;
				inspections[i]++;
			}

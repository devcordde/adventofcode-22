...
func main() {
	...
	//get all dirs
	allDirs := filesystem.GetRootDir().GetDirs(true)

	//loop though all dirs
	for _, currentDir := range allDirs {

		//get how lage the dir is
		dirSize := 0
		for _, file := range currentDir.GetFiles(true) {
			dirSize += file.GetSize()
		}
		//check if the dir is smaller than 100 000
		if dirSize < 100000 {
			sum += dirSize
		}
